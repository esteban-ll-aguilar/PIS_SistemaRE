...
const Sidebar = ({ isVisible, toggleSidebar, role, principal, administrar, acciones, setSelectedComponent, panel=''  }) => {
    ...
    return (
        <div className={`bg-[#04344c] px-[45px] py-[29px] h-screen transition-transform duration-300  ${isVisible ? 'translate-x-0' : '-translate-x-full'} fixed shadow-2xl dark:bg-blue-950`}>
            ...
            <div className='pt-[15px] border-b-[1px] border-[#EDEDED]/[0.3]'>
                {principal.map((item, index) => (
                    <div key={index} className='flex items-center justify-between gap-[10px] py-[15px] hover:bg-[#0491d1] cursor-pointer rounded-lg' onClick={() => handleClick(item.ruta)}>
                        <div className='flex items-center gap-[10px]'>
                        {item.icono}
                        <Link className='text-[14px] text-white leading-[20px] font-normal hover:bg-[#0491d1]'>{item.texto}</Link>
                        </div>
            ...
            <div className='pt-[15px] border-b-[1px] border-[#EDEDED]/[0.3]'>
                ...
                {administrar.map((item, index) => (
                    <div key={index} className='flex items-center justify-between gap-[10px] py-[15px] cursor-pointer hover:bg-[#0491d1] rounded-lg' onClick={() => handleClick(item.ruta)}>
                        <div className='flex items-center gap-[10px]'>
                        {item.icono}
            ...
            <div className='pt-[15px] border-b-[1px] border-[#EDEDED]/[0.3]'>
                ...
                {acciones.map((accion, index) => (
                    <div key={index} className='flex items-center justify-between gap-[10px] py-[15px] cursor-pointer hover:bg-[#0491d1] rounded-lg' onClick={() => handleClick(accion.ruta)}>
                        <div className='flex items-center gap-[10px]'>
                        {accion.icono}
